...
class And_AST : public Binary_AST
{
	private:
		llvm::Value* build(llvm::Value* lval, llvm::Value* rval){
			return Builder.CreateAnd(lval, rval, "tempAnd");
		}
	public:
		And_AST() 
			:Binary_AST("And") {}
...
class Or_AST : public Binary_AST
{
	private:
		llvm::Value* build(llvm::Value* lval, llvm::Value* rval){
			return Builder.CreateOr(lval, rval, "tempOr");
		}
	public:
		Or_AST() 
			:Binary_AST("Or") {}
...
static Block_Stack block_stack;
...
class Field_Decl_AST : public Decaf_AST
{
	...
	string str() { 
		return string("FieldDecl(" ) + name_ + "," + type_->str() + "," + size_ +")";
	}
	...
	llvm::Value *codegen() {
		/*
			 llvm::AllocaInst *alloca;
			 llvm::Type *type = type_->gen_global();
			 alloca = Builder.CreateAlloca(type, nullptr, name_);
			 return alloca;
			 */
		return NULL;
	}
